...
public class AuthenticationAuditBuilder extends AuditBuilder<AuthenticationAuditBuilder> {
    ...
    public AuthenticationAuditBuilder principal(Authentication principal) {
        setActor(null, EntityType.USER, (String) principal.getPrincipal(), null);
        if (principal.getAdditionalInformation() != null) {
            if (principal.getAdditionalInformation().containsKey("ipAddress")) {
                ipAddress((String) principal.getAdditionalInformation().get("ipAddress"));
            }
            if (principal.getAdditionalInformation().containsKey("userAgent")) {
                userAgent((String) principal.getAdditionalInformation().get("userAgent"));
            }
        }
        return this;
    }
    ...
    public AuthenticationAuditBuilder user(User user) {
        setActor(user.getId(), EntityType.USER, user.getUsername(), getDisplayName(user));
        return this;
    }
