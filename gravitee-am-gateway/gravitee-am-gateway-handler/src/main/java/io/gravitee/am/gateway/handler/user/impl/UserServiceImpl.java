                ...
                .doOnSuccess(user1 -> {
                    ...
                    principal1.getAdditionalInformation()
                            .put(StandardClaims.NAME,
                                    user1.getDisplayName() != null ? user1.getDisplayName() :
                                            (user1.getFirstName() != null ? user1.getFirstName() + (user1.getLastName() != null ? " " + user1.getLastName() : "") : user1.getUsername()));
                    auditService.report(AuditBuilder.builder(UserAuditBuilder.class).domain(domain.getId()).client(client.getClientId()).principal(principal1).type(EventType.FORGOT_PASSWORD_REQUESTED));
                })
                .doOnError(throwable -> auditService.report(AuditBuilder.builder(UserAuditBuilder.class).domain(domain.getId()).client(client.getClientId()).principal(principal).type(EventType.FORGOT_PASSWORD_REQUESTED).throwable(throwable)))
                .toCompletable();
