    ...
    public void send(Email email, User user, Client client) {
        if (enabled) {
            try {
                ...
                auditService.report(AuditBuilder.builder(EmailAuditBuilder.class).domain(domain.getId()).client(client.getClientId()).email(email).user(user));
            } catch (final Exception ex) {
                LOGGER.error("Error while sending email", ex);
                new TechnicalManagementException("Error while sending email", ex);
                auditService.report(AuditBuilder.builder(EmailAuditBuilder.class).domain(domain.getId()).client(client.getClientId()).email(email).throwable(ex));
            }
        }
    }
