...
public class UserAuthenticationManagerImpl implements UserAuthenticationManager {
    ...
    public Single<User> authenticate(Client client, Authentication authentication) {
        ...
        return Observable.fromIterable(client.getIdentities())
                ...
                .doOnSuccess(user -> auditService.report(AuditBuilder.builder(AuthenticationAuditBuilder.class).principal(authentication).domain(domain.getId()).client(client.getClientId()).user(user)))
                .doOnError(throwable -> auditService.report(AuditBuilder.builder(AuthenticationAuditBuilder.class).principal(authentication).domain(domain.getId()).client(client.getClientId()).throwable(throwable)));
    }
